...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const firebaseResult = await authService.signUpWithEmail(email, password, {
      displayName: name
    });
    
    if (firebaseResult.success) {
      // Sync with backend
      const syncResult = await authService.syncUserWithBackend(firebaseResult.user);
      
      if (syncResult.success) {
        localStorage.setItem("token", syncResult.data.token);
        localStorage.setItem("user", JSON.stringify(syncResult.data.user));
        navigate(syncResult.data.user?.role === "volunteer" ? "/volunteer/dashboard" : "/dashboard");
      } else {
      ...
      const res = await fetch("/api/auth/register", {
        ...
        body: JSON.stringify({ name, email, password, phone, role, district }),
      });
      if (res.ok) {
        ...
        alert(data.error || firebaseResult.error || "Registration failed");
      }
    }
  };
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      if (result.success) {
        console.log('✅ Register: Google auth successful, checking role selection...');
        ...
        if (roleCheckResult.success) {
          ...
          console.error('❌ Register: Role check failed:', roleCheckResult.error);
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "linear-gradient(120deg, #e0eafc 0%, #cfdef3 100%)" }}>
      <div style={{ background: "#fff", borderRadius: 20, boxShadow: "0 8px 32px rgba(44,62,80,0.13)", padding: "40px 32px", maxWidth: 420, width: "100%", margin: "32px 0" }}>
        <h2 style={{ textAlign: "center", color: "#2d6cdf", fontWeight: 800, fontSize: 32, marginBottom: 24 }}>Create Account</h2>
        <form onSubmit={handleSubmit} style={{ display: "grid", gap: 18 }}>
          ...
          <input type="tel" value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="Phone Number" pattern="[0-9]{10}" required style={inputStyle} />
          <select value={district} onChange={e => setDistrict(e.target.value)} required style={inputStyle}>
